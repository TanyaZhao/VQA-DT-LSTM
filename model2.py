...
class ChildSumTreeLSTM_(nn.Module):

    def __init__(self, vocab_size, in_dim, mem_dim, cuda = True):
        super(ChildSumTreeLSTM_, self).__init__()
        self.cudaFlag = cuda
        self.in_dim = in_dim
...
class Similarity(nn.Module):
    ...
    def forward(self, lvec, rvec):
        mult_dist = F.torch.mul(lvec, rvec)     # 乘性距离
        abs_dist = F.torch.abs(F.torch.add(lvec, -rvec))        # 差距离
        vec_dist = F.torch.cat((mult_dist, abs_dist), 1)        # 拼接
        out = F.sigmoid(self.wh(vec_dist))          # 线性映射
        # out = F.sigmoid(out)
        out = F.log_softmax(self.wp(out))           # 分类num_classes
        return out
...
class SimilarityTreeLSTM(nn.Module):
    ...
    def __init__(self, cuda, vocab_size, in_dim, mem_dim, hidden_dim, num_classes, sparsity):
        ...
        self.cudaFlag = cuda
        self.childsumtreelstm = ChildSumTreeLSTM_(cuda, vocab_size, in_dim, mem_dim, sparsity)
